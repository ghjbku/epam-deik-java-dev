...
public interface ScreeningRepository extends JpaRepository<Screening, Integer> {
    ...
    @Query(value = "SELECT * FROM Screenings scr WHERE "
            ...
            nativeQuery = true)
    Optional<Screening> findByMovieNameRoomNameAndScreeningDate(String movieName,
                                                                String roomName,
                                                                Date screeningDate);
    ...
    @Query(value = "SELECT * FROM Screenings scr WHERE "
            ...
            + "AND EXISTS (SELECT 1 FROM Rooms r WHERE r.name = scr.room_Name AND r.name = ?2) ",
            nativeQuery = true)
    Optional<Screening> findByMovienameAndRoomname(String movieName, String roomName);

    @Query(value = "SELECT * FROM Screenings scr WHERE "
            + "EXISTS (SELECT 1 FROM Rooms r WHERE r.name = scr.room_Name AND r.name = ?1) ",
            nativeQuery = true)
    Optional<List<Screening>> findByRoomName(String roomName);
